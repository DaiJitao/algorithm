...
import numpy as np
from dl.backend import get_available_gpus
from dl.layers import *
from dl.snippets import insert_arguments
from dl.snippets import delete_arguments
from dl.snippets import is_string, string_matching
from dl.snippets import orthogonally_resize
...
class UniLM_Mask(object):
    ...
    def compute_attention_bias(self, inputs=None):
        ...
        if self.attention_bias is None:
            ...
            self.attention_bias = self.apply(
                inputs=self.inputs[1], #  [<tf.Tensor 'Input-Token:0' shape=(None, None) dtype=float32>, <tf.Tensor 'Input-Segment:0' shape=(None, None) dtype=float32>]
                layer=Lambda,
                function=unilm_mask,
...
if __name__ == '__main__':
    import os
    from dl.tokenizers import Tokenizer
    from dl.snippets import to_array

    gpt_config_path = './gpt_config.json'
    gpt2_model = build_transformer_model(config_path=gpt_config_path, model='gpt2')

    model_path = 'D:/myData/ml/nlp/models/chinese/chinese_L-12_H-768_A-12'
